...
vi.mock('./procedure-utils', async (importOriginal) => {
  ...
  return {
    ...
    ProcedureUtils: vi.fn(class extends SharedUtils<unknown> {
      ...
      key = vi.fn(() => ({ key: true }))
      asyncDataArgs = vi.fn(() => ({ asyncDataArgs: true }))
...
describe('createRouterUtils', () => {
  ...
  it('creates nested procedure & shared utils', () => {
    ...
    expect(utils.matcher()(generateOperationKey([], { prefix: '__prefix__' }))).toBe(true)
    expect(utils.matcher()(generateOperationKey([]))).toBe(false)
    expect(utils.asyncDataArgs()).toBe(vi.mocked(ProcedureUtils).mock.results[0]?.value.asyncDataArgs.mock.results[0]?.value)
    ...
    expect(keyUtils.matcher()(generateOperationKey(['key'], { prefix: '__prefix__' }))).toBe(true)
    expect(keyUtils.asyncDataArgs()).toBe(vi.mocked(ProcedureUtils).mock.results[0]?.value.asyncDataArgs.mock.results[0]?.value)
    ...
    expect(pongUtils.matcher()(generateOperationKey(['key', 'pong'], { prefix: '__prefix__' }))).toBe(true)
    expect(pongUtils.asyncDataArgs()).toBe(vi.mocked(ProcedureUtils).mock.results[0]?.value.asyncDataArgs.mock.results[0]?.value)
  })
  ...
  it('works with plain object routers', () => {
    ...
    expect(utils.asyncDataArgs).toBeUndefined()
    expect(utils.matcher()(generateOperationKey([]))).toBe(true)

    const nestedUtils = utils.nested

    expect(ProcedureUtils).toHaveBeenCalledTimes(0)
    expect(nestedUtils.matcher()(generateOperationKey(['nested']))).toBe(true)
    ...
    expect(pingUtils.asyncDataArgs()).toBe(vi.mocked(ProcedureUtils).mock.results[0]?.value.asyncDataArgs.mock.results[0]?.value)
  })
