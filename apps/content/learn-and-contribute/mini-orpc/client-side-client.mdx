...
sidebar:
  label: "3. Client-side Client"
ai:
  exclude: true
---

In Mini oRPC, the client-side client initiates remote procedure calls to the server. Both client and server must follow shared conventions to communicate effectively. While we could use the [RPC Protocol](/docs/rpc/protocol), we'll implement simpler conventions for clarity.
