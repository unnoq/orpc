...
| `UpstashRateLimiter`    | ✅            | [Upstash Rate Limit](https://npmx.dev/package/@upstash/ratelimit)                                    |
...
When the same `limiter` and `key` combination is used multiple times in a single request chain, the `ratelimit` middleware performs the rate limit check only once. This behavior follows the [Dedupe Middleware Best Practice](/docs/recipes/dedupe-middleware). To disable deduplication, set `dedupe: false`.
:::
