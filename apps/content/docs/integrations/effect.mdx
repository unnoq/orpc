...
Failure types like `Error` can also hold `ORPCError` instances at runtime. When the error channel contains such types, these utilities accept any code and handlers also receive those hidden errors, so their `data` widens to `unknown`. On the client, combine with [isInferableError](/docs/client/error-handling#using-safe-and-isinferableerror) when you need fully typed data.
