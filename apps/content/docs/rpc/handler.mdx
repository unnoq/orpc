...
- Use an independent protection, such as the [Safe Method CSRF Protection Plugin](/docs/plugins/safe-method-csrf-protection)
...
const handler = new RPCHandler(router, {
  ...
  plugins: [
    new SafeMethodCsrfProtectionHandlerPlugin(), // reject GET requests that may carry SameSite=Lax cookies
  ],
})
```

The plugin rejects the navigations another site can use to send `SameSite=Lax` cookies, such as link clicks and redirects, while cookie-less requests such as `fetch` keep working. See the [Safe Method CSRF Protection Plugin](/docs/plugins/safe-method-csrf-protection) page for its cookie requirements.
