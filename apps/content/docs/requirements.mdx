...
- **Web Streams are missing.** Hermes has no `TransformStream`, so importing oRPC throws `ReferenceError: TransformStream is not defined` before any of your code runs. [Expo](https://docs.expo.dev/versions/latest/sdk/expo/) has installed `ReadableStream`, `WritableStream`, and `TransformStream` as native globals since [SDK 53](https://expo.dev/changelog/sdk-53), which fixes it. On bare React Native, add [web-streams-polyfill](https://github.com/MattiasBuelens/web-streams-polyfill) and load it from your entry file, before importing oRPC. [react-native-polyfill-globals](https://github.com/acostalima/react-native-polyfill-globals) does not replace it: that package installs `ReadableStream` only.
