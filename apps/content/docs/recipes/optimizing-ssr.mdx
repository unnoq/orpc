...
But how? A naive runtime check with a static import would pull your router into the client bundle. The trick is a conditional import guarded by a flag your bundler resolves at build time, so server-only code never reaches the browser.
...
The `typeof window` guard is resolved at build time: Next.js replaces it during bundling, so the client bundle never contains `orpc.server.ts`. With this setup, importing `client` from `lib/orpc.ts` uses the internal-link client during SSR and the remote client in the browser.

:::tip
On Vite-based frameworks, guard the import with `import.meta.env.SSR` instead. See the [Next.js](/docs/adapters/next#optimize-ssr), [Nuxt](/docs/adapters/nuxt#optimize-ssr), [SolidStart](/docs/adapters/solid-start#optimize-ssr), [SvelteKit](/docs/adapters/svelte-kit#optimize-ssr), and [TanStack Start](/docs/adapters/tanstack-start#optimize-ssr) adapter pages for framework-specific setups.
:::
