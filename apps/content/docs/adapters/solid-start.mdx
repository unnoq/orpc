...
:::warning
Import `orpc.server.ts` only from server entry points like `src/entry-server.tsx`. A `typeof window` check inside `orpc.ts` is not enough: the bundler still emits the dynamically imported module, including your router, as a publicly downloadable client chunk.
:::
