...
Neither `Response` implementation on Expo native parses `multipart/form-data`. Both `expo/fetch` and React Native implement only the `application/x-www-form-urlencoded` branch of `Response.formData()`, so a multipart body never decodes back into fields.

The two links hit that differently:

- The **fetch link** encodes multipart through `expo/fetch`, which is correct, and decodes it through `Response.formData()`, which is not. So nested uploads work once the polyfill is in place, and nested downloads do not.
- The **WebSocket link** both encodes and decodes through the global `Response`, which Expo leaves as React Native's. That implementation cannot read a `FormData` body as a blob either, so nested binary fails in both directions.
