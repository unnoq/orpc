...
**The easy fix is to avoid multipart.** Keep files at the root of a procedure's input or output instead of inside an object, or extend the [RPC JSON Serializer](/docs/rpc/serializer) to send them as `base64`.

**The full fix is a `Response` that reads multipart.** The WebSocket link takes one from `globalThis.Response`. The fetch link does not, because it reads whatever object `expo/fetch` returned, so wrap that yourself:

```ts
// SpecResponse: any Response whose formData() reads multipart
const link = new RPCLink({
  origin: process.env.EXPO_PUBLIC_SERVER_URL,
  url: '/rpc',
...
Neither Expo nor oRPC ships one, so you would bring your own.
