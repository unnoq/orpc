...
:::info
Batching is specific to the RPC protocol, so the `handler` is an [RPCHandler](/docs/rpc/handler). Either adapter serves it, the Node.js one above or the [Fetch](/docs/adapters/fetch-api) one, as long as it runs on Node.js, where `zlib` lives.
:::
