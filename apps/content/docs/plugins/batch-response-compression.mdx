...
Use `BatchResponseCompressionHandlerPlugin` to compress them anyway. Registering it is how you state that your batches are mostly compressible, which is usually the case when they carry JSON. It covers every successful batch response, framed or plain JSON, so batch compression no longer depends on which shape a batch happens to take.
