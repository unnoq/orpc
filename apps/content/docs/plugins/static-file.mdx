...
Content types are detected from the file extension with [mime](https://www.npmjs.com/package/mime), and text types are served as UTF-8 so the browser never has to guess an encoding. Use `mimeTypes` to override or add to the detection.
