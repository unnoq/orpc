...
:::warning[Requests without an `Origin` header]
Link clicks, address-bar navigations, GET form submissions, and passive loads such as `<img>` send no `Origin`, so no allowlist entry can admit them. A POST form submission does send one and can be allowlisted.
:::
