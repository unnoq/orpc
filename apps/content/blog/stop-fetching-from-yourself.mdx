...
The fix is to keep the server side of the wiring out of the client bundle. The shared module imports server code only behind a build-time guard, then looks up a well-known global and falls back to the browser client:
...
This time the guard is resolved at build time: in the client build the branch is statically dead, so the bundler drops it, module and all, and the client bundle contains only the shared module. Load order takes care of itself, since the import runs before the `client` export evaluates. And if registration is ever skipped, the fallback touches `window` on the server and throws immediately, instead of silently fetching from yourself again.

:::tip
`import.meta.env.SSR` is statically replaced by Vite and by Next.js's Turbopack. On webpack builds, guard with `typeof window === 'undefined'` instead, which Next.js also replaces at build time.
:::
...
SSR data fetching has a default shape, HTTP to yourself, that exists only because it is the shortest code. The Fetch API's real gift to servers is that a handler is a callable function, and callable functions do not need sockets to be called. Wire the internal client through `globalThis`, guard the server import with a flag your bundler resolves at build time, and the same one-line `import { client }` does the right thing everywhere.

If you use oRPC, this whole pattern is prepackaged: [Optimizing SSR](/docs/recipes/optimizing-ssr) shows both the internal [RPC Link](/docs/rpc/link) and the even leaner [server-side client](/docs/client/server-side), fully typed end to end. If you are building your own, steal the global.
