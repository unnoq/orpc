...
:::info
Calling your own HTTP endpoint during SSR works fine for this tutorial, but it does a needless network round trip inside one process. Once things click, see [Optimizing SSR](/docs/best-practices/optimizing-ssr) for calling the router directly in-process, with zero HTTP involved.
:::
...
- **Skip HTTP during SSR** with the in-process client from the [Optimizing SSR](/docs/best-practices/optimizing-ssr) guide.
